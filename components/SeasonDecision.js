 ...
 */
export const determineCurrentSeason = async () => {
  // ⭐ 파라미터 stationId 제거
  try {
    ...
    if (!locationInfo || !locationInfo.stationId) {
      console.error(
        'SeasonDecision: 관측소 ID를 포함한 위치 정보를 가져오지 못했습니다.',
      );
      return null;
    }
    ...
    if (pastTempData && pastTempData.list) {
      ...
      console.log(
        `[계절 판단] ✅ 계절 판단 성공: ${season} 가중치를 사용합니다.`,
      );
      return season;
    } else {
    ...
    console.error(
      'SeasonDecision: 계절을 판단하는 중 오류가 발생했습니다.',
      error,
    );
    return null;
  }
