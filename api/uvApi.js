...
export const fetchUvIndexForcast = async (areaNo = '4117300000') => { 
  ...
  if (data?.response?.body?.items?.item) {
    console.log(`[자외선 예보 API] ✅ 요청 성공: ${timeString} 데이터 수신 완료.`);
...
export const fetchCurrentUvIndex = async (areaNo = '4117300000') => {
  ...
  if (forecastData && forecastData.hourlyUv && forecastData.uvBaseDate) {
    ...
    console.log(`4. 계산된 최종 키: ${finalKey} (기준시간 ${uvBaseDate.getHours()} + 경과 ${hoursSinceBase})`);
    console.log('--------------------------');
    ...
    if (currentUvIndex !== undefined) {
      console.log(`[현재 자외선] ✅ 조회 성공: 현재 자외선 지수는 [${currentUvIndex}] 입니다.`);
      return currentUvIndex;
    } else {
      console.error(`[현재 자외선] ❌ 에러: 계산된 시간(${hoursSinceBase}h)에 해당하는 자외선 데이터가 없습니다.`);
      return null;
    }
  } else {
    console.error("[현재 자외선] ❌ 에러: 예보 데이터를 가져오는 데 실패했습니다.");
    return null;
  }
};